...
import { membersDance, changeDance, createDance, deleteDance } from './dance'
...
export default {
  ...
  startWebsocket (url) {
    ...
    this.socket.onmessage = msg => {
      ...
      if (TYPE === 'sync') {
        ...
        membersDance(JSON.parse(msg.data), this.active)
      } else if (TYPE === 'change') {
  ...
  activateReposiroty () {
    ...
    return git(`fetch origin ${branch}`, { cwd: path })
      ...
      .then(id =>
        fetch('https://api.github.com/users/' + id.replace(/\n/g, ''))
      )
      .then(res => res.json())
      .then(
        json => {
