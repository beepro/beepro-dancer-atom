...
export default {
    ...
    atom.project.onDidChangeFiles(events => {
      for (const event of events) {
        ...
        if (event.action === 'created') {
          ...
          // TODO implement logic for update binary file.
          // let isTextFile = this.isTextFile(event.path);
  ...
  registerActiveText() {
    ...
    if (editor.getPath() != null) {
      ...
      console.log('path from registerActiveText', path);
  ...
  changeDance(dance){
    console.log('changeDance');
    const absolutePath = `${atom.project.getPaths()[0]}/${dance.path}`;
    const file = fs.readFileSync(absolutePath);
    console.log('origin', file.toString());
    console.log('new', dance.change);
