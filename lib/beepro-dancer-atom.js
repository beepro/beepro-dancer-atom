...
import {createText, deleteText, moveText, changeText, createHoneyComb} from './text.js';
...
export default {
  ...
  createNewTextFile(textEditor) {
    ...
    if (textEditor.getPath() != null) {
      createText.path = textEditor.getPath().replace(atom.project.getPaths()[0], '.');
      console.log('path', createText.path);
      if (this.socket) {
        this.socket.send(JSON.stringify(createText));
        console.log('send createText');
      }
