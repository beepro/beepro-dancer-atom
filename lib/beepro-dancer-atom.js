...
export default {
  ...
  activateBeePro() {
    ...
    atom.project.onDidChangeFiles(events => {
      for (const event of events) {
        ...
        let isTextFile = true;
        fs.readFileSync(event.path, (error, buffer) => {
          console.log('jschardet', jschardet.detect(buffer));
          console.log('encoding', jschardet.detect(buffer).encoding);
          if ( jschardet.detect(buffer).encoding == null) {
            isTextFile = false;
            console.log('isTextFile-After', isTextFile);
          }
        });
  ...
  uploadFile(path) {
    ...
    const requestheader = new Headers();
    requestHeader.append('encrypt', 'multipart/form-data');
    const filePath = path.replace(atom.project.getPaths()[0]);
