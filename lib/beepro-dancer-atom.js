    ...
    atom.project.onDidChangeFiles(events => {
      for (const event of events) {
        ...
        if (event.action === 'created') {
          ...
          this.moveFile(
            event.oldPath.replace(atom.project.getPaths()[0], '.'),
            event.path.replace(atom.project.getPaths()[0], '.')
          )
    ...
    if (editor != null) {
      ...
      if (editor.getPath() != null) {
        ...
        editor
          ...
          .onDidChange(
            event =>
              console.log('registerActiveText', event) ||
              this.assignment(event.changes, path)
          )
      }
    }
  },
  ...
  createNewTextFile (textEditor) {
    if (textEditor.getPath() != null) {
      ...
      textEditor
        ...
        .onDidChange(
          event =>
            console.log('createNewTextFile', event) ||
            this.assignment(event.changes, path)
        )
    }
  },
