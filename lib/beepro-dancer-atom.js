...
export default {
  ...
  registerActiveText () {
    ...
    if (editor != null) {
      ...
      if (editor.getPath() != null) {
        ...
        this.changeBuffer = atom.workspace.getActiveTextEditor().getBuffer().onDidChange(({changes}) => this.assignment(changes, path, 'registerActiveText'))
      }
    }
  },
  ...
  createNewTextFile (textEditor) {
    if (textEditor.getPath() != null) {
      ...
      this.createBuffer = textEditor.getBuffer().onDidChange(({changes}) => this.assignment(changes, path, 'createNewTextFile'))
    }
  },

  assignment (textBuffer, path, from) {
  ...
  startWebsocket (url) {
    ...
    this.socket.onmessage = msg => {
      ...
      if (TYPE === 'sync') {
        ...
        this.changeBuffer.dispose()
        // this.changeDance2(JSON.parse(msg.data))
        this.changeDance2(JSON.parse(msg.data)).then(() => {
          // POC
          const absolutePath = `${atom.project.getPaths()[0]}/${JSON.parse(msg.data).path}`
          atom.workspace.getActiveTextEditor().getBuffer().loadSync(absolutePath, 'utf8')
          atom.workspace.getActiveTextEditor().save()
          this.changeBuffer = atom.workspace.getActiveTextEditor().getBuffer().onDidChange(({changes}) => this.assignment(changes, JSON.parse(msg.data).path, 'reactiveActiveText'))
          // this.registerActiveText()
        })
        console.log('this.changeBuffer2', this.changeBuffer)
  ...
  changeDance2 (dance) {
    return new Promise(resolve => {
      console.log('this.changeBuffer', this.changeBuffer)
