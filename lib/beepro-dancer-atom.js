...
import { addDanceQue, dance, deleteDance, changedByMe, deleteByMe, createByMe, moveByMe} from './dance.js';
    ...
    atom.project.onDidChangeFiles(events => {
      for (const event of events) {
        ...
        if (event.action === 'created') {
          ...
          this.deleteFile(event.path.replace.replace(atom.project.getPaths()[0], '')
                                   .replace(/^\//g,''));
        } else if (event.action === 'renamed') {
          console.log("file is renamed");
          this.moveFile(event.oldPath.replace(atom.project.getPaths()[0], '')
                                     .replace(/^\//g,''),
                        event.path.replace(atom.project.getPaths()[0], '')
                                     .replace(/^\//g,''););
