...
export default {
  ...
  registerActiveText () {
    ...
    if (editor != null) {
      ...
      if (editor.getPath() != null) {
        ...
        this.changeBuffer = editor.getBuffer().onDidChange(({changes}) => this.assignment(changes, path, 'registerActiveText'))
      }
    }
  },
  ...
  createFile (path) {
    ...
    if (socket && createByMe(createText)) {
  ...
  deleteFile (path) {
    ...
    if (socket && deleteByMe(path)) {
      socket.send(JSON.stringify(deleteText))
  ...
  moveFile (oldPath, newPath) {
    ...
    moveText.to = newPath
    if (socket && moveByMe(moveText)) {
      // send(socket, moveText)
      socket.send(JSON.stringify(moveText))
  ...
  assignment (textBuffer, path, from) {
    ...
    if (socket && changedByMe(changeText)) {
      // send(socket, moveText);
      console.log('send changeText')
      socket.send(JSON.stringify(changeText))
      console.log('send changeText')
    }
  ...
  startWebsocket (url) {
    ...
    socket.onmessage = msg => {
      ...
      if (TYPE === 'sync') {
        ...
        this.changeBuffer.dispose()
        this.changeDance2(JSON.parse(msg.data)).then(() => this.registerActiveText())
  ...
  activateReposiroty () {
    ...
    return git(`fetch origin ${branch}`, { cwd: path })
      ...
      .then(() => {
        git(`config --get-all user.name`, { cwd: path }).then(user => {
          who = user.replace(/\n/g, '')
        })
        console.log('who', who)
      })
